<img width="1997" height="814" alt="image" src="https://github.com/user-attachments/assets/e20e58b2-1761-4eb1-af7a-e5344ef7e640" />
...
> ⚠️ **Agix AOS v1 is in beta.** It's early and actively evolving — expect rough edges and breaking changes between releases. [Issues and feedback](../../issues) are very welcome.
...
![Rust toolchain](https://img.shields.io/badge/rust-toolchain-8b5e34)
...
You bring the model; Agix brings the operating layer around it: cheaper multi-agent
coordination and orchestration discipline, using the same models your CLI already calls.

|  |  |  |  |
|---|---|---|---|
| [**Install**](#install) | [**The agent fleet**](#the-agent-fleet) | [**Your data & privacy**](#security--official-sources) | [**Docs**](docs/DEVELOPMENT.md) |

## Install

**No API key required** if you already have Claude Code or OpenAI Codex — Agix runs your
agents through the CLI you already use, so there's nothing to configure and nothing to pay
per token beyond your existing plan.
...
- Builds a **small Rust component on install** — the intra-agent message bus ships as
  source and compiles at install time (so `agix swarm` and `agix agent serve` work from a
  clean install, cross-architecture).
...
Agix ships eight generic, basic-tier agents — a **team**, not a single assistant. Each
carries an explicit role, trust level, and boundary, so the fleet stays legible and
...
Grow the fleet yourself with `agix agent new <name>` — see
...
```
agix-aos/
├── bin/            # the `agix` CLI entry points
├── lib/            # runtime, message bus client, gbrain, soul, onboarding
│   └── agix-runtime.mjs   # the seam: same agent.mjs runs local or in the cloud
├── agents/         # the agent fleet — one directory per agent (manifest + agent.mjs + policy)
├── services/       # coordination MCP server + shared Go components for the bus
├── bench/          # reproducible endurance + efficiency benchmarks
└── test/           # CLI, runtime, and agent test suites
```

The `lib/agix-runtime.mjs` seam is what lets one `agent.mjs` run both locally (via
`launchd`) and in the cloud (via a container) without changing the agent — the runtime,
not the agent, decides where it executes.
...
| **Linux** | **Supported** — the intra-agent bus uses portable Unix-domain sockets and the install path is cross-architecture by design. (Beta: not yet exercised end-to-end on a Linux runner.) |
...
Dev setup, agent structure, the DCO, and the public-clean release gate are in
